...
def create_element(build_ele, doc):
    # Create Element
    element = Beam(doc).create(build_ele)
...
def move_handle(build_ele, handle_prop, input_pnt, doc):
    ...
    if (handle_prop.handle_id == "BeamHeight"):
        build_ele.RibHeight.value = build_ele.BeamHeight.value - build_ele.TopShHeight.value - build_ele.BotShRectHeight.value - build_ele.BotShTriangHeight.value
        build_ele.HoleHeight.value = (build_ele.BeamHeight.value/2) - 45.5
...
def modify_element_property(build_ele, name, value):
    ...
    if (name == "BeamHeight"):
        change = value - build_ele.TopShHeight.value - build_ele.RibHeight.value - build_ele.BotShTriangHeight.value - build_ele.BotShRectHeight.value
        print(change)
        if (change < 0):
            change = abs(change)
            if (build_ele.TopShHeight.value > 320.):
                if (build_ele.TopShHeight.value - change < 320.):
                    ...
                    change = 0.
            if (change != 0) and (build_ele.BotShTriangHeight.value > 160.):
                if (build_ele.BotShTriangHeight.value - change < 160.):
                    change -= build_ele.BotShTriangHeight.value - 160.
                    build_ele.BotShTriangHeight.value = 160.
                else:
                    build_ele.BotShTriangHeight.value -= change
                    change = 0.
            if (change != 0) and (build_ele.BotShRectHeight.value > 153.):
                if (build_ele.BotShRectHeight.value - change < 153.):
                    change -= build_ele.BotShRectHeight.value - 153.
                    build_ele.BotShRectHeight.value = 153.
                else:
                    build_ele.BotShRectHeight.value -= change
                    change = 0.
        ...
        build_ele.BeamHeight.value = value + build_ele.RibHeight.value + build_ele.BotShTriangHeight.value + build_ele.BotShRectHeight.value
        build_ele.HoleHeight.value = (build_ele.BeamHeight.value/2) - 45.5
    
    elif (name == "RibHeight"):
        build_ele.BeamHeight.value = value + build_ele.TopShHeight.value + build_ele.BotShTriangHeight.value + build_ele.BotShRectHeight.value
        build_ele.HoleHeight.value = (build_ele.BeamHeight.value/2) - 45.5
    
    elif (name == "BotShTriangHeight"):
        build_ele.BeamHeight.value = value + build_ele.TopShHeight.value + build_ele.RibHeight.value + build_ele.BotShRectHeight.value
        build_ele.HoleHeight.value = (build_ele.BeamHeight.value/2) - 45.5
    
    elif (name == "BotShRectHeight"):
        build_ele.BeamHeight.value = value + build_ele.TopShHeight.value + build_ele.RibHeight.value + build_ele.BotShTriangHeight.value
        build_ele.HoleHeight.value = (build_ele.BeamHeight.value/2) - 45.5
        ...
        elif (value < build_ele.BotShRectHeight.value + build_ele.BotShTriangHeight.value + 45.5):
            build_ele.HoleHeight.value = build_ele.BotShRectHeight.value + build_ele.BotShTriangHeight.value + 45.5
...
class Beam():
    ...
    def create(self, build_ele):
        ...
        self._bot_sh_up_height = build_ele.BotShTriangHeight.value
        self._bot_sh_low_height = build_ele.BotShRectHeight.value
        self._bot_sh_height = self._bot_sh_up_height + self._bot_sh_low_height

        if (build_ele.WidthThick.value > min(self._top_sh_width, self._bot_sh_width)):
            build_ele.WidthThick.value = min(self._top_sh_width, self._bot_sh_width)        
        self._rib_thickness = build_ele.WidthThick.value
    ...
    def create_handles(self, build_ele):
        ...
        handle5 = HandleProperties("WidthThick",
                                   ...
                                   [("WidthThick", HandleDirection.point_dir)],
                                   HandleDirection.point_dir, True)
